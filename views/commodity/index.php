...
?>
    <?= Html::beginForm(['commodity/index'],'post',['id'=>'search_form']) ?>
    <div style="margin-bottom: 5px;">
        <?= Html::tag('a','添加',['href'=>'/commodity/create','class'=>'btn-primary pull-right btn']) ?>
...
//        ],
        ...
        [
            ...
            'content'=>function($data){
                $color = '#d1d1d1';
                if($data['statu'] == Commodity::$_AUDIT_ACCESS)
                    $color = 'green';
                else if($data['statu'] == Commodity::$_AUDIT_NOT_ACCESS)
                    $color = 'red';
                return '<span style="color: '.$color.';">'.Commodity::$Audits[$data['statu']].'</span>';
            }
        ],
        [
            ...
            'buttons'=>[
                'order' => function($url,$data,$key){
                    $icon = '<span class="glyphicon glyphicon-list-alt"></span>';
                    $html = '<a style="color:#d1d1d1;" title="放单">'.$icon.'</a>';
                    if($data['statu'] == Commodity::$_AUDIT_ACCESS)
                        $html = '<a href="/commodity-order/create?cid='.$data['id'].'" title="放单">'.$icon.'</a>';
                    return $html;
                },
                ...
                'delete' => function ($url,$data,$key){
                    ...
                    if($data['statu'] != Commodity::$_AUDIT_ACCESS)
                        $html = '<a href="delete?id='.$data['id'].'" title="删除" onclick="return confirm(\'是否删除该商品？\');">'.$icon.'</a>';
                    return $html;
                }
            ],
        ]
